...
float PhongShading(GraphicalContext *gc, int p, Vector N, float dist)
{
  ...
  cos_theta  = InnerProduct(gc->viewdir->V,N);
  if (cos_theta > 1.0) 
    cos_theta=1.0;
...
Vector ObjectNormalOnTheFly(GraphicalContext *gc, Image *image, int po, AdjRel *A)
{
  ...
  if ( n > 2 ) {
    for (i=0; i < n-1; i++) {
      V = CrossProduct(valid_neighbor[i],valid_neighbor[i+1]);
      N.x += V.x;
      N.y += V.y;
      N.z += V.z;
    }
...
void VoxelSplatting(GraphicalContext *gc, Image *image, Point P, int p, float phong_val, float opac)
{
  ...
  if (ValidVoxel(image,v)){
    po  = GetVoxelIndex(image,v); 
    if (gc->surf_render[po].opacity > Epsilon){
